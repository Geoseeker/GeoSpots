...
from GeoApp.forms import RegisterUserForm, LoginForm, AddGeoSpotForm, MyGeoForm, SearchForm, AddMineral
...
class AddMin(View):
    def get(self,request):
        form = AddMineral(initial={'dodane': request.user})
        return render(request, 'addmin.html', {'form':form})
    def post(self, request):
        form = AddMineral(data=request.POST)
        ctx = {'form' : form}
        if form.is_valid():
            name = form.cleaned_data['name']
            description = form.cleaned_data['description']
            ...
            description = description,
            location = location,
            user_collection = request.user)
            return HttpResponseRedirect('collection')
        return render(request, 'addmin.html', ctx)  

# View without data var
class AddCollectionView(LoginRequiredMixin, View):
    def get(self,request):
        form = AddMineral(initial={'dodane': request.user})
        return render(request, 'addmin.html', {'form':form})
    def post(self, request):
        form = AddMineral(request.POST,request.FILES)
        ctx = {'form':form}
        if form.is_valid():
            name = form.cleaned_data['name']
            description = form.cleaned_data['description']
            location = form.cleaned_data['location']
            obraz = form.cleaned_data['obraz']
            Collection.objects.create(
                name=name,
                description=description,
                location=location,
