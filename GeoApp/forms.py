...
locations = GeoSpots.objects.all()
...
class AddMineral(forms.Form):
    name = forms.CharField(label = 'nazwa', max_length=64)
    description = forms.CharField(label = 'opis',widget=forms.Textarea, required=True)
    location = forms.ModelChoiceField(queryset = locations)
    obraz = forms.ImageField()
    dodane = forms.CharField(label='Dodane przez',widget=forms.TextInput(attrs={'readonly':'readonly'}))
